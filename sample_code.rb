# Requires saving with BOM to represent non-ASCII


$bShowDiag = false
$bShowBarcodes = true
$bShowBacktrace = false

$bTestDropBox = true
$bTestBase64 = true
$bTestSamplesLocal = true
$bTestSamplesWeb = true
$bTestUtf8 = true
$bTestUtf8Names = true

$samplesLocal =  File.dirname(__FILE__) + "/"   # script folder
$samplesWeb = "https://wabr.inliteresearch.com/SampleImages/"

class Test
    
    def Test.Run(reader)
        # Read from image in DropBox
        if ($bTestDropBox) then
            Test.Read(reader, "https://www.dropbox.com/s/qcd8zfdvckwwdem/img39.pdf?dl=1")
        end	
        if ($bTestSamplesLocal) then
            # Read local files
            Test.Read(reader, $samplesLocal + "1d.pdf")
        end	
        if ($bTestSamplesWeb) then
            # Read 1D and 2D in a single call
            Test.Read(reader,  $samplesWeb + "1d.pdf" + " | " +  $samplesWeb + "2d.pdf")
        end	
        if ($bTestSamplesWeb) then
            # Read specifying type
            Test.ReadOpt(reader,  $samplesWeb + "postal.pdf", "postal", "", 0)
        end	
        if ($bTestSamplesWeb) then
            # Read Driver License (TBR code)
            Test.Read(reader,  $samplesWeb + "drvlic.ca.jpg")
        end	
        if ($bTestSamplesWeb) then
            # Read from multiple images in a single call
            Test.Read(reader,  $samplesWeb + "1d.pdf" + " | " +  $samplesWeb + "2d.pdf")
        end	
        if ($bTestSamplesWeb) then
            # Read from multi-page TIF file
            Test.Read(reader,  $samplesWeb + "c39.multipage.tif")
        end	
        if ($bTestSamplesWeb) then
            # Read specifying Target Barcode Reader (TBR code)
            Test.ReadOpt(reader,  $samplesWeb + "I25.tbr132.jpg", "", "", 132)
        end	
        if ($bTestSamplesWeb) then
            # Read Adobe® LiveCycle® barcodes and decompress.  (UTF-8 incorrectly displayed by echo(barcode.Text))
            Test.Read(reader,  $samplesWeb + "pdf417.adobe.pdf")
        end	
        if ($bTestUtf8) then
            # Read UTF8-encoded values (UTF-8 incorrectly displayed by echo(barcode.Text))
            Test.Read(reader,  $samplesWeb + "pdf417.utf8.pdf")
        end	
        if ($bTestUtf8Names) then
            # Read from UTF8-named files
            Test.Read(reader,  $samplesWeb + "pdf417.ЙшзщЪфг_Russian.pdf")
        end	
        if ($bTestUtf8Names) then
            Test.Read(reader,  $samplesWeb + "pdf417.بساطة لأنه الأل_Arabic.pdf")
        end	
        if ($bTestBase64) then
            # Read from Base64-encoded image
            base64 = ""
            Test.Read(reader, base64)
        end	
        if ($bTestBase64) then
            #  Read from Base64-encoded local image file
            Test.ReadFileBase64(reader, $samplesLocal + "2d.pdf")
        end	
    end
    
    def Test.ReadFileBase64(reader, file)
        begin
            base64 = Base64.encode64(File.open(file, "rb").read)  
            
            ext = File.extname(file).sub(".", "")
            base64 = "data:image/" + ext + ";base64," + base64
            # Optionally attach suffix with reference file name to be placed in Barcode.File property
            base64 = base64 + ":::" + File.basename(file)
            Test.Read(reader, base64)
        rescue Exception => ex
            puts()
            puts("EXCEPTION: " + ex.message)
            if ($bShowBacktrace) then 
                puts ex.backtrace.join("\n")
            end
        ensure
        end
    end
    
    
    def Test.ShowBarcodes(barcodes)
        if (!$bShowBarcodes)
            return;
        end
        barcodes.each do |barcode|
            puts()
            puts("Barcode Type:" + barcode.Type + "  File:" + barcode.File + "   Page:" + barcode.Page.to_s)
            puts(barcode.Text)
            # puts("DATA: "  + barcode.Data)
            if barcode.Values.length > 0 then
                puts("\n VALUES:")
                barcode.Values.each { |key,value| 	puts(key + " : " + value) }
            end
        end
    end
    
    def Test.Read(reader, image)
        begin
            barcodes = reader.Read(image)
            Test.ShowBarcodes(barcodes)
        rescue Exception => ex
            puts()
            puts("EXCEPTION: " + ex.message)
            if ($bShowBacktrace) then 
                puts ex.backtrace.join("\n")
            end
        ensure
        end
    end
    
    def Test.ReadOpt(reader, image, types, dir, tbr_code)
        begin
            barcodes = reader.ReadOpt(image, types, dir, tbr_code)
            Test.ShowBarcodes(barcodes)
        rescue Exception => ex
            puts()
            puts("EXCEPTION: " + ex.message)
            if ($bShowBacktrace) then 
                puts ex.backtrace.join("\n")
            end
        ensure
        end
    end
end
